...
export const Select = genericTypedMemo(<T extends string>(props: SelectProps<T>) => {
    const { className, label, option, value, readOnly, onChange, ...otherProps } = props;

    const optionList = useMemo(
        () =>
            option?.map((opt) => (
                <option value={opt.value} key={opt.value} className={cl.option}>
                    {opt.content}
                </option>
            )),
        [option],
    ...
    return (
        <div className={cn(cl.Wrapper, mods, [className])}>
            ...
            <select className={cl.select} value={value} onChange={onChangeHandle} disabled={readOnly} {...otherProps}>
                {optionList}
            </select>
        </div>
    );
